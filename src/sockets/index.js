  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_session', async ({ code, username }) => {
      ...
      try {
        ...
        await sessionService.addPlayer(session, username, socket.id);
        socket.join(code);
        socket.data = { code, username };

        socket.emit('joined', { session, username });
    ...
    socket.on('guess', async ({ code, guess }) => {
      ...
      try {
        ...
        const { correct, player, attemptsLeft, allOut } = await gameService.processGuess(session, socket.id, guess);
        ...
        if (correct) {
          ...
          io.to(code).emit('game_over', {
            ...
            answer: session.answer,
            players: session.players,
            message: `😔 All players are out of attempts! The answer was: "${session.answer}"`,
          });
          const rotated = await sessionService.rotateGameMaster(session);
          io.to(code).emit('new_game_master', {
            gameMaster: rotated.gameMaster,
    ...
    socket.on('chat', ({ code, message }) => {
      ...
      io.to(code).emit('chat_message', { type: 'chat', username, message: message.trim() });
    });
